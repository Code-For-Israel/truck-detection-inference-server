...
# Count variable for sending a post request
count = 0
sum_process_time = 0
...
while True:
    ...
    try:
        ...
        continue
    count += 1
    process_time = datetime.datetime.now() - current_time
    sum_process_time += process_time.total_seconds()
    print(f'{count} response from backend (sending detection_results of {len(cameras)} cameras in '
          f'{process_time} seconds (average: {round(sum_process_time/count, 2)})): {response}')
